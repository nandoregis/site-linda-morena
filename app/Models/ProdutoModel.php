...
class ProdutoModel
{
    ...
    public function updateStock($dados) {
        return $this->db->connect()->prepare("UPDATE `tb_produtos` SET quantidade = ? WHERE id_code = ?")->execute($dados);
    }
    
    public function deleteImageOfDatabase($idImage) {
        return $this->db->connect()->prepare("DELETE FROM `tb_imagens` WHERE id=?")->execute([$idImage]);
    }
}
